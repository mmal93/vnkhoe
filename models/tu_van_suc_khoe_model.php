...
class Tu_van_suc_khoe_Model extends Model
{
	...
	public function getData($id = '', $limit = 0) {
		...
		if(isset($id) && (!empty($id))) {
			$sth = $this->db->prepare("SELECT tvsk_id, tvsk_hinhanh, tvsk_tieude, tvsk_tomtat,tvsk_noidung,tvsk_ngaydangtin,tvsk_tinhot,tvsk_view, author_name, author_logo, author_description FROM tbl_tvsk, tbl_author WHERE tbl_tvsk.author_id=tbl_author.author_id and tbl_tvsk.tvsk_id = ".$id.$lm);
		}else {
